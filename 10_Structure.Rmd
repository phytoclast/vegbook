...
veg.tern <- veg.tern |> cbind(vegnasis::totriangle(a=veg.tern$tree, b=veg.tern$shrub))
```
...
vtern(background = F) + 
  geom_polygon(data=str.polys, aes(fill=group, x=x, y=y),alpha=0.25,size=0.1, color='black') +
  geom_point(data=veg.tern, aes(x=x, y=y))+
